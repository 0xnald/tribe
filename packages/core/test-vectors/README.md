...
| `distribution.json` | 20      | HardCap / WaterFill / ConditionalCap / Proportional on identical inputs, cascades, dust, empty winners                                                                                                                      |
| `upset.json`        | 8       | formula, reserve balance, draw limit, absolute cap, empty reserve, zero pool                                                                                                                                                |
| `arena.json`        | 9       | full event streams: happy path, TIE + sponsor refund + sweep, guard rejections, cancel-expired, extension + authority cancel, upset + settlement clamp, short arena, water-fill with rollover, ScaledUi split at settlement |
