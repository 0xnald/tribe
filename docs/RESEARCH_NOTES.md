...
are inputs to ARCHITECTURE.md; re-verify anything marked _(re-check)_ before
...
- Solana receiver program id `rec5EKMGg6MxZYaMdyBfgwp4d5rB9T1VQH5pJv5LtFJ` _(re-check)_;
...
0.0007`); `lite-api.jup.ag/swap/v2/order` returned 404 _(re-check whether
  keyless access is stable; assume a key is required in production)_.
