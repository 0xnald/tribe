...
| 16  | Arithmetic overflow / precision                                    | `u128` with `checked_*`; prices normalised to Q10 (u64, ≈ $1.84e9 max); winner rule and tie band in U256; unit-seconds bounded by `u64 × 2^31`        | program + core                    | overflow tests at max plausible values (1e18 units × 30 days)                                                  |
...
| Arithmetic                              | `checked_*`/`mul_div` (U256) everywhere; `overflow-checks = true` in release; Q10 normalisation bounds prices to u64; winner cross products and tie band in U256.                                                                                                                                                                                                                    | vector parity (`cargo test`), overflow vector                                                                                                                                                                               |
...
3. The TS engine mapped a price-scale overflow to `OracleUnsupportedExponent`; the
